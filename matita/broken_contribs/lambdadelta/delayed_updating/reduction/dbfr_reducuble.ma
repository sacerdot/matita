...
lemma dbfr_inv_reducuble (t1) (t2) (r):
...
lemma dbfr_des_reducuble_neq (t1) (t2) (r) (s):
      t1 ϵ 𝐓 → t1 ➡𝐝𝐛𝐟[r] t2 →
      s ⧸= r → s ϵ 𝐑❨t1❩ → s ϵ 𝐑❨t2❩.
#t1 #t2 #r #s #Ht1 #Ht #Hr * #p #b #q #n #H0 #Hb #Hq #Hn destruct
elim (dbfr_inv_reducuble … Ht) #p0 #b0 #q0 #n0 #H0 #_ #_ #Hn0 destruct
@(prc_mk … Hq) [| // ] -Hb -Hq
@(dbfr_des_in_comp_neq … Ht) // -t2 #H0
...
lemma dbfr_des_reduct (t1) (t2) (r) (s):
...
lemma dbfr_des_reducuble_eq (t1) (t2) (r):
      t1 ϵ 𝐓 → t1 ➡𝐝𝐛𝐟[r] t2 →
      r ⧸ϵ 𝐑❨t2❩.
#t1 #t2 #r #Ht1
