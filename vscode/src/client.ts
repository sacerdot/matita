...
export function activate(context: ExtensionContext) {
    ...
    const lspServerPath = workspace.getConfiguration('matita').path;
    ...
    let clientOptions: LanguageClientOptions = {
        documentSelector: [{ scheme: 'file', language: 'ma' }],
    };
    ...
    const restart = () => {
        ...
        client.onReady().then( () => {
            ...
            */
            commands.registerCommand('extension.matita.cursor', () => toggleCursorMode(context));
 
            //Navigate proof : step forward in a proof 
            commands.registerCommand('extension.matita.fw', () => checkProofForward(context));
            
            //Navigate proof : step backward in a proof
            commands.registerCommand('extension.matita.bw', () => checkProofBackward(context));

            //Navigate proof : move proof highlight to cursor
            commands.registerCommand('extension.matita.tc', () => checkProofUntilCursor(context));
            
            //Window follows proof or not
            commands.registerCommand('extension.matita.reveal', () => toggleFollowMode(context))

            //Center window on proof state
            commands.registerCommand('extension.matita.center', () => goToProofState(context));

            //Go to next/previous proof
            commands.registerCommand('extension.matita.nx', () => nextProof(context, true));
            commands.registerCommand('extension.matita.pv', () => nextProof(context, false));
    ...
    commands.registerCommand('extension.matita.restart', restart);
