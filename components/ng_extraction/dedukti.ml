...
let extract_sort ty =
  match ty with App (Const (_, s), t) when s = "Univ" -> t | _ -> assert false
...
and is_prod_product ty =
  match ty with
  | App (App (App (App (Const (_, s), _), _), _), Lam (_, _, ty))
    when s = "prod" ->
      is_sort_product ty
  | _ -> false

let get_sort_product ty =
  match ty with App (App (Const (_, _), s), _) -> s | _ -> assert false

let extract_type ty =
  match ty with App (App (Const (_, _), _), ty) -> ty | _ -> assert false
...
let app_bindings m bs =
  let translate_var x =
    ...
    else if is_sort_product ty then Var x
    else Var x
  in
