...
let find_root path =
  let path = absolutize ~dir:(Sys.getcwd ()) path in
...
(* FG
let debug_paths ~include_paths file =
  HLog.debug ("We are in: " ^ Sys.getcwd ());
  HLog.debug ("Searching for: "^file^"\nPaths explored:");
  List.iter (fun x -> HLog.debug (" - "^x)) include_paths
*)

let find_root_for ~include_paths file =
  let include_paths = "" :: Sys.getcwd () :: include_paths in
   let find_path_for file =
(*      debug_paths ~include_paths file; *)
...
*)
let read_include_paths root_path =
  try
    let root_contents = load_root_file (Filename.concat root_path "root") in
    let includes =
      Str.split (Str.regexp " ") (List.assoc "include_paths" root_contents)
    in
    List.map (absolutize ~dir:root_path) includes
  with Not_found -> []
