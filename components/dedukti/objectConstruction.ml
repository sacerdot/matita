...
let cic_Term =
  Kernel.Basic.mk_name
    (Kernel.Basic.mk_mident "cic")
    (Kernel.Basic.mk_ident "Term")

let cic_lift =
  Kernel.Basic.mk_name
    (Kernel.Basic.mk_mident "cic")
    (Kernel.Basic.mk_ident "lift")

let cic_prod =
  Kernel.Basic.mk_name
    (Kernel.Basic.mk_mident "cic")
    (Kernel.Basic.mk_ident "prod")

let cic_Univ =
  Kernel.Basic.mk_name
    (Kernel.Basic.mk_mident "cic")
    (Kernel.Basic.mk_ident "Univ")
...
let cic_type =
  Kernel.Basic.mk_name
    (Kernel.Basic.mk_mident "cic")
    (Kernel.Basic.mk_ident "type")

let cic_prop =
  Kernel.Basic.mk_name
    (Kernel.Basic.mk_mident "cic")
    (Kernel.Basic.mk_ident "prop")

let cic_z =
  Kernel.Basic.mk_name
    (Kernel.Basic.mk_mident "cic")
    (Kernel.Basic.mk_ident "z")

let cic_succ =
  Kernel.Basic.mk_name
    (Kernel.Basic.mk_mident "cic")
    (Kernel.Basic.mk_ident "s")

let rec calc_univ_dept = function
  | Kernel.Term.Const (_, name) when Kernel.Basic.name_eq name cic_z -> 0
  | Kernel.Term.App (Kernel.Term.Const (_, f_name), a, [])
    when Kernel.Basic.name_eq f_name cic_succ ->
      1 + calc_univ_dept a
...
let make_type_n_uri term =
  ...
  if univ_dept >= 0 && univ_dept <= 5 then
    ...
    failwith_log
      (Format.sprintf "Univers number must be between 0 and 5. Got %d" univ_dept)

let rec construct_debrujin index = NCic.Rel (index + 1)

and construct_type term =
  NCic.Sort (NCic.Type [ (`Type, make_type_n_uri term) ])
...
and construct_const ~baseuri:_ name =
  ...
  | None ->
      failwith_log
        ("name "
        ^ Kernel.Basic.string_of Kernel.Basic.pp_name name
        ^ " not found in const table")

and construct_sort = function
  | Kernel.Term.App (Kernel.Term.Const (_, name), a1, [])
    when Kernel.Basic.name_eq name cic_type ->
      construct_type a1
...
and construct_appl ~baseuri f a1 args =
  ...
  | ( Kernel.Term.Const (_, name),
      [ _; _; Kernel.Term.Lam (_, ident, Some typ, body) ] )
  ...
  | Kernel.Term.Const (_, name), [ _; _; Kernel.Term.Lam (_, _, None, _) ]
    when Kernel.Basic.name_eq name cic_prod ->
      assert false
  | Kernel.Term.Const (_, name), []
    when Kernel.Basic.name_eq name cic_univ
         || Kernel.Basic.name_eq name cic_Univ ->
      construct_sort a1
  | Kernel.Term.Const (_, name), _
    when Kernel.Basic.mident_eq (Kernel.Basic.md name) cic_cic ->
      assert false
  | _ ->
...
and construct_term ~baseuri = function
  ...
  | Kernel.Term.Lam (_, ident, Some typ, body) ->
      construct_lambda ~baseuri (Kernel.Basic.string_of_ident ident) typ body
  | Kernel.Term.Lam (_, _, None, _) -> assert false
  | Kernel.Term.Pi (_, ident, typ, body) ->
      construct_prod ~baseuri (Kernel.Basic.string_of_ident ident) typ body
...
let construct_constant status ~baseuri ident typ body =
  ...
  let name =
    Kernel.Basic.mk_name
      (Kernel.Basic.mk_mident (Filename.basename baseuri))
      ident
  in
  let uri = mkuri ~baseuri str_ident "con" in
  let obj_kind = construct_constant_kind ~baseuri typ body str_ident in
  let height =
    NCicTypeChecker.height_of_obj_kind status uri ~subst:[] obj_kind
  in
  let reference =
    NReference.reference_of_spec uri
      (if body <> None then NReference.Def height else NReference.Decl)
  in
  assert (not (Hashtbl.mem const_tbl name));
  Hashtbl.add const_tbl name reference;
  (uri, height, [], [], obj_kind)

let rec extract_idents_from_pattern = function
  | Kernel.Rule.Var (_, ident, _, []) -> [ Kernel.Basic.string_of_ident ident ]
  | Kernel.Rule.Var (_, ident, _, pat_list) ->
      let str_ident = Kernel.Basic.string_of_ident ident in
      let others =
        List.flatten
          (List.map (fun pat -> extract_idents_from_pattern pat) pat_list)
      in
      str_ident :: others
...
let construct_fixpoint_function ~baseuri (typ_entry, body_entry, attrs) =
  ...
  | Parsers.Entry.Decl (_, _, _, _, typ), Parsers.Entry.Rules (_, rule_list) ->
      let typ' = construct_term ~baseuri typ in
      let body' =
        construct_fixpoint_body ~baseuri (List.hd rule_list) typ' recno
      in
      ([], name, recno, typ', body')
...
let construct_fixpoint status ~baseuri fixpoint_list =
  let names =
    List.map
      (fun (typ, _, (_, leftno)) -> (name_of_decl ~baseuri typ, leftno))
      fixpoint_list
  in
  ...
  List.iteri
    (fun i (name, leftno) ->
      let reference =
        NReference.reference_of_spec uri (NReference.Fix (i, leftno, 0))
      in
      if Hashtbl.mem const_tbl name then
        failwith_log
          ("Fixpoint "
          ^ Kernel.Basic.string_of Kernel.Basic.pp_name name
          ^ "already registed in const table")
      else Hashtbl.add const_tbl name reference)
    names;
  let functions =
    List.map (construct_fixpoint_function ~baseuri) fixpoint_list
  in
...
let construct_inductive_constructor ~baseuri block_uri indtyno leftno consno =
  function
  | Parsers.Entry.Decl (_, ident, _, _, term) ->
      ...
      let reference =
        NReference.reference_of_spec block_uri
          (NReference.Con (indtyno, consno + 1, leftno))
      in
      if Hashtbl.mem const_tbl name then
        failwith_log
...
let construct_inductive_type ~baseuri block_uri leftno indtypno
    (typ, conss, attrs) =
  match typ with
  | Parsers.Entry.Decl (_, _, _, _, typ_term) ->
      ...
      let conss' =
        List.mapi
          (construct_inductive_constructor ~baseuri block_uri indtypno leftno)
          conss
      in
      ([], name, typ', conss')
  | _ -> assert false
...
let construct_inductive status ~baseuri leftno types =
  ...
  List.iteri
    (fun i name ->
      let reference =
        NReference.reference_of_spec uri (NReference.Ind (true, i, leftno))
      in
      if Hashtbl.mem const_tbl name then
        failwith_log
          ("Inductive type "
...
let rec read_until_end_pragma pragma_name buf =
  try
    ...
    | Parsers.Entry.Pragma (_, str)
      when PragmaParsing.is_valid_export_pragma str
...
let handle_pragma_block status ~baseuri buf pragma_string =
  ...
  | Some export_pragma -> (
      ...
      | PragmaParsing.FixpointPragma fixpoint_list ->
          construct_fixpoint status ~baseuri fixpoint_list
...
let obj_of_entry status ~baseuri buf = function
  ...
  | Parsers.Entry.Pragma (_, str) ->
      if
        ...
        && String.starts_with "PRAGMA BEGIN" str
      then handle_pragma_block status ~baseuri buf str
      else (
