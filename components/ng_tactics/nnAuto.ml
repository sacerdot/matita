...
let noprint ?depth:(_depth=0) _s = ()
let debug_print = ref noprint
...
let print_stat _status tbl =
  ...
  let short_name r =
(*    Filename.chop_extension *) (* FG: This causes an error because of the uri fragment *)
...
let fast_eq_check eq_cache status (goal:int) =
  ...
  | [] -> raise (Error (lazy "fast_eq_check: no proof found",None))
  | s::_ -> s
;;
...
let paramod eq_cache status goal =
  ...
  | [] -> raise (Error (lazy "paramod: no proof found",None))
  | s::_ -> s
;;
...
(* FG: for now we catch TypeCheckerFailure; to be understood *)
      | NCicTypeChecker.TypeCheckerFailure _ ->
          ...
          raise (Error (lazy "smart_apply: no proof found",None))
;;
...
*)
  ...
  let time  = int "time" flags 300 in
  NCicParamod.time := float time;
  Paramod.max_steps := int "steps" flags 2;
  debug_print := if List.mem_assoc "debug" flags then print else noprint;
